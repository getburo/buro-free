...
Depth beyond `SKILL.md`'s summary. Open this when actually seating a table — the mechanics of
RIFF vs FAN, the three-chair discipline, the riff protocol round by round, and the canonical
tables live here so `SKILL.md` only needs to state that the mechanism exists and when to reach
for it.
