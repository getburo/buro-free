...
<sub>Free tier · 16 seats · last updated 2026-08-03</sub>

**A studio's craft method, written down and packaged as a family of Claude Code skills — 16
full-strength seats, at least one in every department (16 of the full studio's 59), each with its
own method and a critique that pushes back, shipped at full depth. No thinning: every included
seat directs and produces exactly as it does in the full studio. The rest of each department's
bench lives in the full commercial Buro studio, a separate commercial product.**

**What this is, plainly:** plain-text files you can read. There is no model in here, no
inference, no calls to any model provider — nothing is generated by Buro itself. The files are
read by Claude Code, which you license directly from Anthropic; that subscription is not
...
them — not to copy, modify, distribute, sublicense, or create derivative works. No open-source or
future-conversion license applies. Anything you *produce* using Buro is entirely yours, with no
restriction; the method files themselves are not for redistribution.
